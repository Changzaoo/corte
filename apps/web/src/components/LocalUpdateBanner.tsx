...
export default function LocalUpdateBanner() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        const [v, latest] = await Promise.all([api.systemVersion(), latestServerSha()])
        if (!alive || !v.local || !latest) return
        remoteSha.current = latest
        // sem carimbo local (instalação antiga) também conta como desatualizado
        setOutdated(!v.sha || v.sha !== latest)
      } catch { /* backend fora do ar / rota antiga — silencioso */ }
    }
    check()
